...
const main = () => {
  const spreadsheet = SpreadsheetApp.openByUrl(
    'https://docs.google.com/spreadsheets/d/1oYNekgQ5bqJ69QM2AfHenOZjNGH5NlukmORksPo-ooI/edit#gid=0'
  );
  ...
  for (let i = beginRow; i <= range.getLastRow(); i++) {
    const date: Date = range.getCell(i, 1).getValue();
    const text: string = range.getCell(i, 2).getValue();
    const groupId: string = range.getCell(i, 3).getValue();
 ...
 * LINE にテキスト送信する
 */
const sendToLine = (to: string, text: string) => {
